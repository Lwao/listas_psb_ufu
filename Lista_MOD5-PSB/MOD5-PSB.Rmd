...
# plots
dygraph(df, main = "Linear interpolation to remove discontinuities linear para remoção de descontinuidades") %>% 
  dySeries(name="ch1", drawPoints=TRUE, color = "pink" ) %>%
  dySeries(name="ch2", drawPoints=TRUE, color = "cyan")   %>%
  dySeries(name="ch1_int", drawPoints=TRUE, color = "red")  %>% 
  dySeries(name="ch2_int", drawPoints=TRUE, color = "blue")  %>% dyOptions(pointSize = 2)
